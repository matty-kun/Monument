  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full h-full min-h-0">
      ...
      <div className="lg:col-span-4 space-y-8 h-full md:overflow-y-auto pr-1 custom-scrollbar pb-10 md:pb-0">
        <div className="bg-white dark:bg-[#1c1c1e] rounded-[24px] shadow-sm border border-gray-200 dark:border-white/5 overflow-visible transition-all flex flex-col">
          <div className="p-6 border-b border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-[#1c1c1e] shrink-0 z-10 rounded-t-[24px]">
            <h2 className="text-[12px] font-bold uppercase tracking-widest text-gray-500 dark:text-white/40">Create New Account</h2>
          </div>
          ...
          <div className="p-6">
            <form onSubmit={handleCreateUser} className="space-y-6">
              <div>
                ...
                <input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="w-full bg-white dark:bg-white/5 text-gray-900 dark:text-white border border-gray-200 dark:border-white/5 rounded-2xl px-4 py-4 text-sm font-bold placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-2 focus:ring-[#0A84FF] focus:border-transparent transition-all outline-none" placeholder="name@example.com" required />
              </div>

              <div className="relative">
                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Access Password</label>
                <input type={showNewPassword ? "text" : "password"} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full bg-white dark:bg-white/5 text-gray-900 dark:text-white border border-gray-200 dark:border-white/5 rounded-2xl pl-4 pr-12 py-4 text-sm font-bold placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:ring-2 focus:ring-[#0A84FF] focus:border-transparent transition-all outline-none" placeholder="••••••••" required />
              ...
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">System Role</label>
      ...
      <div className="lg:col-span-8 flex flex-col h-full overflow-hidden">
        <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-[#1c1c1e] p-2 rounded-[24px] border border-gray-200 dark:border-white/5 shadow-sm gap-4 mb-6">
           <div className="relative flex-1 w-full">
              <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
              <input type="text" placeholder="Search users by email or role..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-gray-100 dark:bg-white/5 text-gray-900 dark:text-white border-none rounded-[16px] pl-12 pr-4 py-3 text-sm font-medium outline-none focus:ring-1 focus:ring-gray-300 dark:focus:ring-white/20 transition-all placeholder:text-gray-400 dark:placeholder:text-gray-500" />
           </div>
        </div>
                  ...
                  <tr key={user.id} className="group hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-8 py-5 text-right">
                      {user.id !== currentUserId && (
                        <button onClick={() => handleDeleteClick(user)} className="p-2 text-gray-500 dark:text-white/40 hover:text-[#FF453A] hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-all"><FaTrash size={14} /></button>
                      )}
                    </td>
                  </tr>
                ))}
