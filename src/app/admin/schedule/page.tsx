          ...
          <div className="h-full overflow-y-auto custom-scrollbar p-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-32">
              {filteredSchedules.map((s) => {
                ...
                return (
                  <div key={s.id} className="h-fit bg-white dark:bg-[#1c1c1e] rounded-[24px] shadow-sm border border-gray-200 dark:border-white/5 overflow-hidden flex flex-col hover:border-gray-300 dark:hover:border-white/20 transition-all duration-300 relative group">
                    <div className="px-5 py-3.5 flex justify-between items-center border-b border-gray-200 dark:border-white/5 bg-gray-50/50 dark:bg-white/[0.02]">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500 dark:text-white/40 flex items-center gap-2">
                    ...
                    <div className="p-6 space-y-5 flex-1">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 flex items-center justify-center bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 rounded-[12px] text-2xl drop-shadow-sm">{s.events?.icon || '🏅'}</div>
                        <div className="flex flex-col min-w-0">
                          <h3 className="text-[18px] font-bold text-gray-900 dark:text-white tracking-tight truncate">{formatEventName(s.events)}</h3>
                          <p className="text-[11px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest mt-0.5">
                            {typeof s.events?.category === 'object' && s.events.category ? s.events.category.name : categories.find(c => c.id === s.events?.category)?.name || ''}
                          </p>
                        </div>
                      </div>

                      <div className="bg-gray-50 dark:bg-white/5 rounded-[20px] p-4 border border-gray-200 dark:border-white/5">
                        <div className="flex items-center justify-around gap-2 relative min-h-[56px]">
                          {s.departments.length === 0 ? (
                            <div className="flex flex-col items-center py-2 opacity-50">
                               <div className="w-14 h-14 rounded-[16px] bg-gray-100 dark:bg-white/5 border border-dashed border-gray-300 dark:border-white/10 flex items-center justify-center text-[10px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest mb-1 shadow-inner">
                                  ?
                               </div>
                               <span className="text-[9px] font-bold uppercase text-gray-500 dark:text-white/40 tracking-[0.2em]">TBA Teams</span>
                            </div>
                            ...
                            return (
                              <Fragment key={i}>
                                <div className="flex flex-col items-center gap-2 relative">
                                  ...
                                  <div className={`w-14 h-14 rounded-full flex items-center justify-center text-[11px] font-bold transition-all border shadow-sm ${isWinner ? 'bg-yellow-400/20 text-yellow-600 dark:text-yellow-500 border-yellow-400/30 scale-110' : 'bg-white dark:bg-white/5 text-gray-600 dark:text-white/60 border-gray-200 dark:border-white/10'}`}>
                                    {d?.image_url ? (
                                  ...
                                  <span className={`text-[10px] font-bold uppercase tracking-widest ${isWinner ? 'text-yellow-600 dark:text-yellow-500' : 'text-gray-600 dark:text-white/60'}`}>
                                    {d?.abbreviation || dName?.slice(0,3) || "??"}
                                  </span>
                                </div>
                                {i < s.departments.length - 1 && <span className="text-[10px] font-black text-gray-400 dark:text-white/20 italic">vs</span>}
                              </Fragment>
                            );
                          })}
                        </div>
                        ...
                        {s.status === 'finished' && (Number(s.score_a) > 0 || Number(s.score_b) > 0) && (
                          <div className="flex items-center justify-center gap-4 mt-4 pt-3 border-t border-gray-200 dark:border-white/10 font-bold text-2xl text-gray-900 dark:text-white">
                            <span className={s.winner_id === (departmentMap.get(s.departments[0])?.id) ? 'text-[#34C759]' : 'text-gray-500 dark:text-white/40'}>{s.score_a || 0}</span>
                            <span className="text-gray-300 dark:text-white/20">—</span>
                            <span className={s.winner_id === (departmentMap.get(s.departments[1])?.id) ? 'text-[#34C759]' : 'text-gray-500 dark:text-white/40'}>{s.score_b || 0}</span>
                          </div>
                        )}
                      </div>
                    ...
                    {!selectedTournament?.is_archived && (
                      <div className="bg-gray-50 dark:bg-[#1c1c1e] px-6 py-3 border-t border-gray-200 dark:border-white/5 flex justify-end items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                         {dynStatus.status !== 'finished' ? (
                           s.departments.length > 0 ? (
                              ...
                              <div className="w-9 h-9 bg-gray-100 dark:bg-white/5 text-gray-400 dark:text-white/30 rounded-[12px] flex items-center justify-center border border-dashed border-gray-300 dark:border-white/10 cursor-not-allowed" title="Assign teams to finish match">
                                 <CheckCircle2 size={14} />
                              </div>
                           )
                         ) : (
                           <button onClick={() => handleResetMatch(s.id)} className="w-9 h-9 bg-white dark:bg-white/5 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-white/10 border border-gray-200 dark:border-white/10 rounded-[12px] flex items-center justify-center shadow-sm transition-all" title="Reset Match Status">
                         ...
                         {s.departments.length > 0 ? (
                          ...
                          <div className="w-9 h-9 bg-gray-100 dark:bg-white/5 text-gray-400 dark:text-white/30 rounded-[12px] flex items-center justify-center border border-dashed border-gray-300 dark:border-white/10 cursor-not-allowed" title="Assign teams first to manage results">
                             <ClipboardEdit size={16} />
                          </div>
                         )}
                         <button onClick={() => { 
      ...
      <AnimatePresence>
        {showFormModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-xl bg-white dark:bg-[#1c1c1e] rounded-[24px] shadow-2xl border border-gray-200 dark:border-white/5">
              <div className="px-8 pt-8 pb-4 flex justify-between items-center border-b border-gray-200 dark:border-white/10">
                <div>
                  <h2 className="text-gray-900 dark:text-white font-bold text-xl uppercase tracking-tight">{editingId ? 'Edit Schedule' : 'Create New Schedule'}</h2>
                </div>
                <button onClick={closeModal} className="w-8 h-8 flex items-center justify-center bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-white/50 hover:text-gray-900 dark:hover:text-white rounded-full hover:bg-gray-200 dark:hover:bg-white/10 transition-all active:scale-95"><X size={16} strokeWidth={3} /></button>
              </div>
      ...
      <AnimatePresence>
        {showConfirmModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-sm bg-white dark:bg-[#1c1c1e] rounded-[32px] p-8 shadow-2xl border border-gray-200 dark:border-white/5 space-y-6">
              ...
              <div className="text-center space-y-2">
                <h3 className="text-gray-900 dark:text-white font-bold uppercase tracking-tight text-lg">Erase match record?</h3>
                <p className="text-[11px] font-bold text-gray-500 dark:text-white/50 uppercase tracking-widest">This action is permanent and will remove the schedule from all views.</p>
              </div>
              <div className="flex gap-3 mt-8">
                <button onClick={() => setShowConfirmModal(false)} className="flex-1 py-3 text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-white/50 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 hover:text-gray-900 dark:hover:text-white rounded-[16px] transition-all">Abort</button>
