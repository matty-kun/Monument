      ...
      <div className="flex flex-col flex-1 min-h-0 pb-2">
        <div className="flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-[#1c1c1e] p-4 rounded-[24px] border border-gray-200 dark:border-white/5 shadow-sm gap-4 shrink-0 mb-4">
           <div className="relative flex-1 w-full">
              ...
              <input type="text" placeholder="Search categories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-gray-100 dark:bg-white/5 text-gray-900 dark:text-white border-none rounded-2xl pl-12 pr-4 py-3 text-sm font-medium focus:ring-2 focus:ring-monument-primary/20 outline-none placeholder:text-gray-400 dark:placeholder:text-white/40 transition-all" />
           </div>
                      ...
                      <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => { setCategoryName(c.name); setEditingId(c.id); setShowFormModal(true); }} className="p-2 text-gray-500 dark:text-white/40 hover:text-yellow-500 hover:bg-yellow-50 dark:hover:bg-yellow-500/10 rounded-xl transition-all" title="Edit Category"><FaEdit /></button>
                            <button onClick={() => { setCategoryToDeleteId(c.id); setShowConfirmModal(true); }} className="p-2 text-gray-500 dark:text-white/40 hover:text-[#FF453A] hover:bg-[#FF453A]/10 rounded-xl transition-all" title="Delete Category"><FaTrash /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      <ConfirmModal
        ...
        message={
          <div className="space-y-6 pt-4 text-left">
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-white/40 mb-2 italic">Category Name</label>
              <input 
                 type="text" 
                 value={categoryName} 
                 onChange={(e) => setCategoryName(e.target.value)}
                 className="w-full bg-gray-50 dark:bg-white/5 border-none rounded-2xl px-4 py-4 text-sm font-bold text-gray-900 dark:text-white shadow-inner placeholder:text-gray-400 dark:placeholder:text-white/30 focus:ring-2 focus:ring-monument-primary transition-all outline-none" 
